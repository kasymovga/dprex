...
static int R_LoadSkyBox(void)
{
	...
	for (j=0; j<3; j++)
	{
		...
		for (i=0; i<6; i++)
		{
			...
			skyboxskinframe[i] = R_SkinFrame_LoadInternalBGRA(name, TEXF_CLAMP | (gl_texturecompression_sky.integer ? TEXF_COMPRESS : 0), temp, image_width, image_height, 0, 0, 0, vid.sRGB3D);
